...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Tracks the current user id so we can ignore Supabase's TOKEN_REFRESHED events
  // (fired on every tab refocus) when the user hasn't actually changed — re-fetching
  // there causes a loading flash / apparent page refresh each time you return.
  ...
  useEffect(() => {
    ...
    const {
      ...
      if (!mounted) return;
      const currentUser = session?.user ?? null;
      // Same user (e.g. TOKEN_REFRESHED on tab refocus, or the initial-session echo)
      // → do nothing. Re-fetching here is what made the page "refresh" every time you
      // came back to the tab.
      if (currentUser?.id && currentUser.id === lastUserIdRef.current) return;
      lastUserIdRef.current = currentUser?.id ?? null;
      setUser(currentUser);
