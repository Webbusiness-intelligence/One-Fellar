  ...
  return (
    <section className="flex h-full flex-col rounded-xl border border-border bg-card">
      ...
      <footer className="flex items-center gap-4 border-t border-border px-5 py-3 text-xs text-muted-foreground">
        <LegendDot color="#1f9d6b" label="Incoming" />
        <LegendDot color="#6b7a99" label="Outgoing" />
      </footer>
    </section>
  )
}
        ...
        {hover !== null && (
          <g pointerEvents="none">
            <line
              ...
              strokeDasharray="3 3"
            />
            <circle cx={hoverX} cy={yFor(data[hover.idx].incoming)} r={3.5} fill="#1f9d6b" />
            <circle cx={hoverX} cy={yFor(data[hover.idx].outgoing)} r={3.5} fill="#6b7a99" />
          </g>
        )}
      </svg>
